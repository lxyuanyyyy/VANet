...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
        self.dim = dim
        self.window_size = window_size  # Wd, Wh, Ww 是相等的
        self.query_size = self.window_size
        self.key_size = self.window_size[0] * 2  # why??
        ...
        coords = torch.stack(torch.meshgrid([coords_d, coords_h, coords_w]))  # 2, Wh, Ww
...
class GlobalAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, input_resolution, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0.,
        ...
        self.seq_len = h // self.query_size  # TODO

        self.num_heads = num_heads
        head_dim = dim // num_heads
        self.scale = qk_scale or head_dim ** -0.5
        self.reduction = 32
        self.pre_conv = nn.Conv3d(dim, int(dim // self.reduction), 1)

        # define a parameter table of relative position bias
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((2 * self.seq_len - 1) * (2 * self.seq_len - 1) * (2 * self.seq_len - 1), num_heads))  # 2*Wh-1 * 2*Ww-1, nH
        # print(self.relative_position_bias_table.shape)
        # get pair-wise relative position index for each token inside the window
        coords_d = torch.arange(self.seq_len)
        ...
        relative_coords[:, :, 2] += self.seq_len - 1

        relative_coords[:, :, 0] *= (2 * self.seq_len - 1) ** 2
        relative_coords[:, :, 1] *= 2 * self.seq_len - 1

        relative_position_index = relative_coords.sum(-1)  # Wh*Ww, Wh*Ww

        self.register_buffer("relative_position_index", relative_position_index)

        self.queryembedding = Rearrange('b c (d p3) (h p1) (w p2) -> b (p1 p2 p3 c) d h w',
                                        p1=self.query_size, p2=self.query_size, p3=self.query_size)

        self.keyembedding = nn.Conv3d(self.seq_len, self.key_size ** 3 * self.seq_len, kernel_size=self.key_size, stride=4, padding=1)

        self.query_dim = int(dim // self.reduction) * self.query_size * self.query_size * self.query_size
        self.key_dim = int(dim // self.reduction) * self.key_size * self.key_size * self.key_size
    ...
    def forward(self, x, D, H, W):
        ...
        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(
            self.seq_len * self.seq_len * self.seq_len, self.seq_len * self.seq_len * self.seq_len, -1)  # Wh*Ww,Wh*Ww,nH
        relative_position_bias = relative_position_bias.permute(2, 0, 1).contiguous()  # nH, Wh*Ww, Wh*Ww
...
class LocalTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = window_reverse(attn_windows, self.window_size, D, H, W)  # B H' W' C
        x = x.view(B, D, H, W, C)[:, 0:D-pad_d, 0:H-pad_b, 0:W-pad_r, :].contiguous().view(B, -1, C)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        if downsample is not None:
            if min(self.input_resolution) >= self.window_size:
                self.glb_attn = GlobalAttention(dim, to_3tuple(window_size), self.input_resolution, num_heads=num_heads,
                                                qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop,
                                                proj_drop=drop)
                self.post_conv = nn.Conv3d(dim, dim, 3, padding=1)
                self.norm1 = norm_layer(dim)
                self.norm2 = norm_layer(dim)
    ...
    def forward(self, x):
        ...
        if self.downsample is not None:
            ...
            if min(self.input_resolution) >= self.window_size:
                ...
                no_window = int(D * H * W / self.window_size ** 3)
                local_attn = x.view(B, no_window, self.window_size, self.window_size, self.window_size, C)

                glb_attn = self.glb_attn(x, D, H, W)
                glb_attn = glb_attn.view(B, no_window, 1, 1, 1, C)
                x = torch.add(local_attn, glb_attn).view(B, C, D, H, W)

                x = shortcut.view(B, C, D, H, W) + self.drop_path_gl(x)
                x = self.norm2(x.view(B, D * H * W, C))
                post_conv = self.drop_path_gl(self.post_conv(x.view(B, C, D, H, W))).view(B, D * H * W, C)
                x = x + post_conv
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size, patch_size, in_chans=3, embed_dim=96, norm_layer=None):
        super().__init__()
        ...
        padding = tuple(np.array(patch_size) // 2 - 1)
...
class MOATransformer(nn.Module):
    ...
    def __init__(self, img_size=(192, 192, 80), patch_size=(16, 16, 4), in_chans=1, num_classes=2,
...
if __name__ == "__main__":
    x = torch.randn(2, 1, 192, 192, 80).cuda()
    model = MOATransformer().cuda()
    out = model(x)
    print(out)
