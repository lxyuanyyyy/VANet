...
class PRM(nn.Module):
    def __init__(self, img_size=512, kernel_size=4, downsample_ratio=1, dilations=[1, 3, 7], in_chans=1, embed_dim=64,
        ...
        self.proj = nn.Conv2d(embed_dim*3, embed_dim, 1)

        self.act_layer = nn.GELU()
        self.gap = nn.AdaptiveAvgPool2d((1, 1))
        self.fc1 = nn.Linear(embed_dim, self.embed_dim // 2)
        self.fc2 = nn.Linear(embed_dim // 2, len(self.dilations) * embed_dim)
        self.softmax = nn.Softmax(dim=1)
        self.proj_head = nn.Linear(self.embed_dim, self.embed_dim)
    ...
    def forward(self, x):
        ...
        for i in range(len(self.dilations)):
            yy.append(self.convs[i](x))
        y = torch.stack(yy, dim=1)
        y_ = torch.cat(yy, dim=1)
        B, C, W, H = y_.shape
        feats = self.proj(y_)
        ...
        attention_vectors = attention_vectors.view(B, -1, self.embed_dim, 1, 1)
        attention_vectors = self.softmax(attention_vectors)
        feats_V = torch.sum(y * attention_vectors, dim=1)  # [bs,channel,H,W]
        feats_V = self.proj_head(feats_V.reshape(B, self.embed_dim, H * W).permute(0, 2, 1))  # [bs,H*W,dim]
        feats_V = feats_V.permute(0, 2, 1).reshape(B, self.embed_dim, H, W)
        output = feats + feats_V  # [bs,dim,H,W]
        return output
...
class MHCA_stage(nn.Module):
    ...
    def __init__(
        ...
        self.aggregate = Conv2d_BN(embed_dim * 2,
                                   out_embed_dim,
                                   act_layer=nn.Hardswish)

    def forward(self, inputs):
        """foward function"""
        attn_outputs = [self.InvRes(inputs)]

        _, _, H, W = inputs.shape
        x = inputs.flatten(2).transpose(1, 2)
        attn_outputs.append(self.mhca_blks(x, size=(H, W)))

        out_concat = torch.cat(attn_outputs, dim=1)
