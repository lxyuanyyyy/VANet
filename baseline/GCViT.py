...
class GCViT(nn.Module):
    ...
    def __init__(self,
        ...
        Args:
            dim: feature size dimension.
            depths: 每层的layer个数 number of layers in each stage.
            window_size: 每个stage的window个数 window size in each stage.
            mlp_ratio: MLP ratio.
            num_heads: number of heads in each stage.
            resolution: 图像尺寸?          input image resolution.
...
@register_model
def gc_vit_lvo(pretrained=False):
    model = GCViT(depths=[3, 4, 23, 5],
                  num_heads=[3, 6, 12, 24],
                  resolution=512,
