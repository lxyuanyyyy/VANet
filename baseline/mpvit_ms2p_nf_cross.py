...
class FusBlock(nn.Module):
    def __init__(self, embed_dim, r=4):
        super(FusBlock, self).__init__()

        self.prm = PRM(embed_dim=embed_dim)
        self.maxpool = nn.AdaptiveMaxPool2d(1)
        self.avgpool = nn.AdaptiveAvgPool2d(1)
        self.se = nn.Sequential(
            nn.Conv2d(embed_dim, embed_dim // r, 1, bias=False),
            nn.ReLU(),
            nn.Conv2d(embed_dim // r, embed_dim, 1, bias=False),
        )
        self.sigmoid = nn.Sigmoid()

        self.spatial = Conv(2, 1, 7, bn=True, relu=False, bias=False)
        self.W_l = Conv(embed_dim, embed_dim, 1, bn=True, relu=False)
        self.W_g = Conv(embed_dim, embed_dim, 1, bn=True, relu=False)
        self.aggregate1 = nn.Sequential(
            nn.Conv2d(embed_dim, embed_dim, 1, 1),
            nn.BatchNorm2d(embed_dim)
        )
        self.Avg = nn.AvgPool2d(2, stride=2)
        self.gelu = nn.GELU()
        self.aggregate2 = nn.Sequential(
            nn.Conv2d(embed_dim * 3, embed_dim, 1, 1),
            nn.BatchNorm2d(embed_dim)
        )

    def forward(self, fusion, conv, attn):
        l = self.W_l(conv)
        g = self.W_g(attn)
        fusion = fusion + l + g
        fusion = self.aggregate1(fusion)
        ...
        g = self.sigmoid(output) * attn

        fuse = torch.cat([g, l, fusion], 1)
        fuse = self.aggregate2(fuse)
        fuse = fusion + fuse
        return fuse
...
class Multiscale_Stage(nn.Module):
    def __init__(self, embed_dim, out_embed_dim, num_layers=1, num_heads=8, mlp_ratio=3,
        ...
        self.ga = Global_Attention(embed_dim)
        self.la = Local_Attention(embed_dim)
        self.emb1 = Conv2d_BN(embed_dim, out_embed_dim, 3, 2, 1)
        self.emb2 = Conv2d_BN(embed_dim, out_embed_dim, 3, 2, 1)
        self.emb3 = Conv2d_BN(embed_dim, out_embed_dim, 3, 2, 1)
        self.prm = PRM(in_chans=embed_dim, embed_dim=out_embed_dim, kernel_size=3, downsample_ratio=2)
        self.concat1 = nn.Sequential(
            nn.Conv2d(embed_dim * 3, embed_dim, 1, 1),
            nn.BatchNorm2d(embed_dim)
        )
        self.fusion1 = nn.Conv2d(embed_dim*2, embed_dim, 1, 1)
    ...
    def forward(self, guide):
        ...
        fuse = torch.cat([conv, attn], 1)
        fuse = self.fusion1(fuse)
        loc = self.la(fuse)
        glb = self.ga(fuse)
        conv = conv * loc
        attn = attn * glb
...
class MPViT(nn.Module):
    ...
    def __init__(
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(embed_dims[-1] * 3, embed_dims[-1], 1, 1),
