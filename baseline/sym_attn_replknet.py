...
class RepLKNet(nn.Module):
    ...
    def __init__(self, large_kernel_sizes, layers, channels, drop_path_rate, small_kernel,
        ...
        self.final_conv=conv_bn_relu(1024,1024,1, 1, 0, groups=1)



        # self.sym_block=flip_l1_block(channels[-1],channels[-1])
        self.sym_block = sym_fusion_res_block(channels[-1], channels[-1], abs=False)  # 0.96 acc
        #self.sym_block = sym_fusion_attn_block(channels[-1],channels[-1],abs=False)
        # self.sym_block=res_CBAM(gate_channels=channels[-1])
        # self.sym_block=res_RGA(in_channels=channels[-1],feature_map_size=8*7,abs=False)
        # self.sym_block=res_CA(in_channels=channels[-1],abs=False)
        # self.sym_block=res_TA(in_channels=channels[-1],abs=False)
        # self.sym_block = nn.Identity()
        # 分类任务
    ...
    def forward_features(self, x):
        ...
        '''
        # 获取空间注意力
        spatial_attn_scale = self.spatial_attn(x)
        level_spatial_attn = []
        for i in range(self.num_stages):
            #print(spatial_attn_scale.shape)
            level_spatial_attn.append(spatial_attn_scale)
            spatial_attn_scale = self.spatial_attn_down_sample(spatial_attn_scale)
        '''
        '''
        spatial_attn_scale = self.spatial_attn(x)
        level_spatial_attn = []
...
def sym_attn_RepLKNet31B(drop_path_rate=0.3, num_classes=2, use_checkpoint=False, small_kernel_merged=False,
                         in_ch=1):
...
def create_RepLKNet(drop_path_rate=0.3, num_classes=2, use_checkpoint=False, small_kernel_merged=False, MTL=False):
...
def create_RepLKNetXL(drop_path_rate=0.3, num_classes=2, use_checkpoint=True, small_kernel_merged=False):
...
if __name__ == '__main__':
    model = sym_attn_RepLKNet31B(small_kernel_merged=False).to('cuda:2')
    model.eval()
    print('------------------- training-time model -------------')
    # print(model)
    x = torch.randn(2, 1, 229, 193).to('cuda:2')
    a, b = model(x)
    print(a, b)
