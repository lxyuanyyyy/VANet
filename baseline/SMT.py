...
class Head(nn.Module):
    def __init__(self, head_conv, dim):
        ...
        stem = [nn.Conv2d(1, dim, head_conv, 2, padding=3 if head_conv == 7 else 1, bias=False), nn.BatchNorm2d(dim),
                nn.ReLU(True)]
...
class SMT(nn.Module):
    def __init__(self, img_size=256, in_chans=1, num_classes=2, embed_dims=[64, 128, 256, 512],
