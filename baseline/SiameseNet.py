...
class RepLKNet(nn.Module):
    ...
    def __init__(self, large_kernel_sizes, layers, channels, drop_path_rate, small_kernel,
        ...
        if num_classes is not None:
            ...
            self.coll_head = nn.Linear(channels[-1], 3)
    def siamese_forward(self, x):
        #权重共享
        pass
    def orgin_forward(self,x):
        #双分支输入 原图  或  左半脑
        pass
    def flip_forward(self,x):
        #双分支输入 翻转图  或  右半脑翻转
...
def create_RepLKNet31B(drop_path_rate=0.3, num_classes=2, use_checkpoint=False, small_kernel_merged=False, MTL=False,in_ch=1):
...
if __name__ == '__main__':
    model = create_RepLKNet31B(small_kernel_merged=False).to('cuda:2')
    model.eval()
    print('------------------- training-time model -------------')
    # print(model)
    x = torch.randn(2, 1, 512, 512).to('cuda:2')
    a, b = model(x)
    print(a, b)
