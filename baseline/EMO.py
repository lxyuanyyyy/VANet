...
from .emo_modules import get_norm, get_act, ConvNormAct, LayerScale2D, MSPatchEmb
...
class EMO(nn.Module):
    ...
    def __init__(self, dim_in=1, num_classes=2, img_size=256,
...
def EMO_5M(pretrained=False, **kwargs):
    model = EMO(
        ...
        norm_layers=['bn_2d', 'bn_2d', 'ln_2d', 'ln_2d'], act_layers=['silu', 'silu', 'gelu', 'gelu'],
        dw_kss=[3, 3, 5, 5], dim_heads=[24, 24, 32, 32], window_sizes=[8, 8, 8, 8], attn_ss=[False, False, True, True],
        qkv_bias=True, attn_drop=0., drop=0., drop_path=0.05, v_group=False, attn_pre=True, pre_dim=0,
        **kwargs)
    return model
