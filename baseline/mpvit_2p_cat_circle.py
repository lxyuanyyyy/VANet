...
class MHCA_stage(nn.Module):
    ...
    def __init__(
        ...
        self.fusion = Circle(embed_dim)
        self.aggregate = Conv2d_BN(embed_dim,
                                   out_embed_dim,
                                   act_layer=nn.Hardswish)
    ...
    def forward(self, inputs):
        ...
        cnn_output = self.InvRes(inputs)
        _, _, H, W = inputs.shape
        x = inputs.flatten(2).transpose(1, 2)
        attn_outputs = self.mhca_blks(x, size=(H, W))
        out = self.fusion(cnn_output, attn_outputs)
