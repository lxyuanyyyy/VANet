...
def create_RepLKNet31B(drop_path_rate=0.3, num_classes=2, use_checkpoint=False, small_kernel_merged=False, MTL=False,
                       in_ch=1):
...
if __name__ == '__main__':
    ...
    # print(model)
    x = torch.randn(1, 1, 512, 512).to('cuda:2')
    a= model(x)
    print(a)
    #输出模型参数量
    origin_y = sum(p.numel() for p in model.parameters() if p.requires_grad)
