...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=(192, 192, 80), patch_size=16, in_chans=3, embed_dim=768):
        super().__init__()
        ...
        self.D, self.H, self.W = img_size[0] // patch_size[0], img_size[1] // patch_size[1], img_size[2] // patch_size[
            2]
...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=(192, 192, 80), patch_size=4, in_chans=1, num_classes=2,
        ...
        for i in range(num_stages):
            patch_embed = PatchEmbed(img_size=img_size if i == 0 else tuple([j // 2 ** (i + 1) for j in img_size]),
    ...
    def _get_pos_embed(self, pos_embed, patch_embed, D, H, W):
        if D * H * W == self.patch_embed1.num_patches:
            return pos_embed
        else:
            return F.interpolate(
                pos_embed.reshape(1, patch_embed.D, patch_embed.H, patch_embed.W, -1).permute(0, 4, 1, 2, 3),
                size=(D, H, W), mode="trilinear").reshape(1, -1, D * H * W).permute(0, 2, 1)
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages):
            ...
            if i == self.num_stages - 1:
                ...
                pos_embed_ = self._get_pos_embed(pos_embed[:, 1:], patch_embed, D, H, W)
                pos_embed = torch.cat((pos_embed[:, 0:1], pos_embed_), dim=1)
            else:
                pos_embed = self._get_pos_embed(pos_embed, patch_embed, D, H, W)
