...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=(1, 7, 7), shift_size=0,
        ...
        self.attn = WindowAttention(
            dim, window_size, num_heads=num_heads,
    ...
    def forward(self, x, mask_matrix):
        ...
        shifted_x = window_reverse(attn_windows, self.window_size, Dp, Hp, Wp)  # B H' W' C
...
class BasicLayer(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.window_size = window_size
        self.shift_size = window_size[-1] // 2
        self.depth = depth
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.cls = nn.Linear(embed_dim * 2 ** (self.num_layers-1), num_classes)
    ...
    def forward(self, x):
        ...
        if self.ape:
            ...
            absolute_pos_embed = F.interpolate(self.absolute_pos_embed, size=(Wd, Wh, Ww), mode='bicubic')
        ...
        for i in range(self.num_layers):
            layer = self.layers[i]
            x_out, D, H, W, x, Wd, Wh, Ww = layer(x, Wd, Wh, Ww)

            if i in self.out_indices:
                norm_layer = getattr(self, f'norm{i}')
                x_out = norm_layer(x_out)

                x_out = x_out.view(-1, D, H, W, self.num_features[i]).permute(0, 4, 1, 2, 3).contiguous()

        x = self.avg_pool(x_out).view(B, -1)
        # x = self.cls(x)
        return x
